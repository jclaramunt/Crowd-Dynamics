function [time, xsols, vsols, Mass, l] = Crowds2(n,t,M,Mp,Case,mcase);
tic
r = 1/4;
% masses
if mcase == 1
...
% ode function
    function dwdt = eqn(~,w)
        ...
        %FP(2:n) = (1/10)*((sign(V(1:n-1)-V(2:n))).^2).*...
        %    ((m*Del(2:n)).*((sign(V(1:n-1)-V(2:n))+1)/2)...
        %    -(M*(Del(2:n)-d0)).*((sign(V(1:n-1)-V(2:n))-1)/2)); 
        ...
        FP(2:n)= (P/10)*((V(1:n-1)-V(2:n))>0)...
            +(P/10)*(sign(V(1:n-1)-V(2:n))<=0).*((Del(2:n)-Delta0)>0);
                FP(2:n) = (1/10)*((sign(V(1:n-1)-V(2:n))).^2).*...
            ((mp*Del(2:n)).*((sign(V(1:n-1)-V(2:n))+1)/2)...
            -(Mp*(Del(2:n)-dp)).*((sign(V(1:n-1)-V(2:n))-1)/2)); %la nueva
        %FP(2:n)=0;
